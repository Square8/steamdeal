...
스팀 게임 가격을 원화로 매일 기록해서 **"지금 할인가가 역대 최저인지"** 알려주는 정적 사이트.

스팀 상점은 지금 가격만 보여주고 이게 역대 최저인지 알려주지 않는다. 그 정보가 이 사이트의 가치다.

- **맥북을 켜놓지 않아도 된다.** GitHub Actions 가 하루 2번 클라우드에서 돌린다.
- **호스팅비 0원.** GitHub Pages.
- **스팀 API 키가 필요 없다.** 공식 상점 API는 키 없이 쓸 수 있다.
- **쿠팡 API 없이 시작 가능.** 스팀만으로 돌아간다.
...
GitHub Actions (하루 2번)
   │
   ├─ collect.py   스팀 상점 API → 오늘 가격을 SQLite 에 기록
   │                (추적 대상은 스팀의 특별할인/인기 목록에서 자동 수집)
   ├─ build.py     SQLite → 정적 HTML 생성 (역대최저 계산, 가격추이 차트)
   ├─ 가격 이력 커밋  data/prices.sqlite3 을 저장소에 push
   └─ GitHub Pages 배포
```
