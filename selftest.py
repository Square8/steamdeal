...
print("\n9d) 리뷰 등급 ('매우 긍정적' 등)")
# appreviews 는 텍스트(review_score_desc)를 안 믿고 정수 등급만 받는다 —
# 이 응답 모양은 여기서 검증 불가능하므로(steam.REVIEW_STATS 참고),
# 파싱 자체는 query_summary 가 있든 없든 죽지 않는지만 확인한다.
_rev_payload = {"query_summary": {"review_score": 8, "total_positive": 860,
                                  "total_reviews": 1000}}
_score, _pct = with_fake(_rev_payload, lambda: steam.fetch_review_summary(42))
check("review_score 파싱", _score == 8, str(_score))
check("긍정 비율 계산 (860/1000)", _pct == 86, str(_pct))
_score0, _pct0 = with_fake({"success": 0}, lambda: steam.fetch_review_summary(43))
check("query_summary 없으면 (0,0)", (_score0, _pct0) == (0, 0))
check("전체 0건이면 나눗셈 대신 0%",
      with_fake({"query_summary": {"review_score": 5, "total_positive": 0,
                                    "total_reviews": 0}},
                lambda: steam.fetch_review_summary(44))[1] == 0)

def _g(score, count=500, pct=80):
    return {"review_score": score, "review_count": count, "review_positive_pct": pct}

check("8점(매우 긍정적)은 rev-good", build.review_sentiment(_g(8))[1] == "rev-good")
check("9점(압도적 긍정)도 rev-good", build.review_sentiment(_g(9))[1] == "rev-good")
check("5점(복합적)은 rev-mid", build.review_sentiment(_g(5))[1] == "rev-mid")
check("2점(매우 부정적)은 rev-bad", build.review_sentiment(_g(2))[1] == "rev-bad")
check("등급 텍스트에 긍정 비율 동반 표시",
      "긍정 80%" in build.review_sentiment(_g(8))[0]
      and "매우 긍정적" in build.review_sentiment(_g(8))[0])
check("리뷰가 적으면(문턱 미만) 등급을 안 보여준다 — 등급 자체가 의미 없다",
      build.review_sentiment(_g(8, count=3)) is None)
check("등급 0(미조회)은 아무것도 안 보여준다", build.review_sentiment(_g(0)) is None)

_card_sent = build.card({"appid": 2, "name": "평가있음", "is_free": 0, "history": [],
                         "price_final": 1000, "discount_pct": 0,
                         "review_score": 8, "review_count": 500,
                         "review_positive_pct": 80})
check("카드에 등급 배지가 색+글자로 함께 표시", 'class="tagline rev-good"' in _card_sent
      and "매우 긍정적" in _card_sent)
_card_nosent = build.card({"appid": 3, "name": "평가없음", "is_free": 0, "history": [],
                           "price_final": 1000, "discount_pct": 0})
check("등급 데이터 없는 카드는 배지 클래스 없음", "rev-good" not in _card_nosent
      and "rev-bad" not in _card_nosent)
check("리뷰 등급 색이 최저가(--deal)와 다른 변수를 쓴다",
      "--good:" in _css and "--deal:" in _css and
      _css.count("--good:") >= 1)

print("\n9e) 리뷰 등급 DB 저장/보존")
conn = store.connect()
g_new = dict(app); g_new["appid"] = 5001
g_new.update(review_count=500, review_score=8, review_positive_pct=80)
store.save(conn, g_new, "신작"); conn.commit()
row = conn.execute("SELECT review_score, review_positive_pct FROM games WHERE appid=5001"
                    ).fetchone()
check("리뷰 등급이 저장된다", tuple(row) == (8, 80), str(tuple(row)))
# 다음 갱신에서 리뷰가 문턱 밑이라 등급을 다시 안 물었을 때(0,0으로 옴)
# 예전에 이미 확인해둔 등급을 지우면 안 된다 (review_count 66개 무갱신 버그와 같은 종류).
g_refresh = dict(app); g_refresh["appid"] = 5001
g_refresh.update(review_count=500, review_score=0, review_positive_pct=0)
store.save(conn, g_refresh, None); conn.commit()
row2 = conn.execute("SELECT review_score, review_positive_pct FROM games WHERE appid=5001"
                    ).fetchone()
check("등급을 재조회하지 않은 갱신에서 기존 등급이 보존된다",
      tuple(row2) == (8, 80), str(tuple(row2)))
conn.close()
